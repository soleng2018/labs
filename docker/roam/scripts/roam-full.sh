...
# WiFi Auto Roaming Script for Container Environment
# Enhanced version of the original roam script optimized for Docker containers
# Usage: ./roam.sh <SSID> <min_time_minutes> <max_time_minutes> [min_signal_dbm] [interface]

set -euo pipefail

# Configuration - now using environment variables when available
...
log_info() {
    echo -e "${BLUE}[ROAM-INFO]${NC} $(date '+%Y-%m-%d %H:%M:%S') $1"
}

log_success() {
    echo -e "${GREEN}[ROAM-SUCCESS]${NC} $(date '+%Y-%m-%d %H:%M:%S') $1"
}

log_warning() {
    echo -e "${YELLOW}[ROAM-WARNING]${NC} $(date '+%Y-%m-%d %H:%M:%S') $1"
}

log_error() {
    echo -e "${RED}[ROAM-ERROR]${NC} $(date '+%Y-%m-%d %H:%M:%S') $1"
}

# Function to save roaming statistics
save_roaming_stats() {
    local stats_file="${LOG_DIR}/roaming_stats.json"
    local current_time
    ...
    json_entry=$(cat << EOF
{
  ...
  "available_bssids": ${#AVAILABLE_BSSIDS[@]}
}
EOF
)
    
    # Append to stats file
    echo "$json_entry" >> "$stats_file"
}

# Function to automatically detect wireless interface with container optimizations
auto_detect_interface() {
    local detected_interface=""
    
    # First try to read from config file (set by startup script)
    local interface_file="${CONFIG_DIR}/wifi_interface.txt"
    if [ -f "$interface_file" ]; then
        detected_interface=$(cat "$interface_file" 2>/dev/null || true)
        if [ -n "$detected_interface" ]; then
            log_info "Using interface from config: $detected_interface"
...
# Enhanced main function for container environment
main() {
    ...
    if [ $# -lt 3 ] || [ $# -gt 5 ]; then
        ...
        echo "  SCAN_INTERVAL=${SCAN_INTERVAL:-30}"
        echo "  SIGNAL_THRESHOLD=${SIGNAL_THRESHOLD:--75}"
        exit 1
    fi
    ...
    # Save interface to config file for future use
    echo "$INTERFACE" > "${CONFIG_DIR}/wifi_interface.txt"
    ...
    log_info "Starting Container WiFi SSID-based roaming script"
    ...
    log_info "Container mode: Enhanced logging and monitoring enabled"
    ...
    # Main roaming loop with enhanced container features
    local iteration=1
    ...
    while true; do
        ...
        # Get current BSSID and save stats
        local current_bssid
        current_bssid=$(get_current_bssid)
        local current_signal
        current_signal=$(sudo wpa_cli -i "$INTERFACE" signal_poll 2>/dev/null | grep "^RSSI=" | cut -d'=' -f2 || echo "unknown")
        
        save_roaming_stats "$current_bssid" "$current_signal" "$iteration"
        
        log_info "Currently connected to: ${current_bssid} (Signal: ${current_signal} dBm)"
...
trap 'echo -e "\n${YELLOW}[ROAM-INFO]${NC} Script interrupted by user. Exiting..."; exit 0' INT TERM
